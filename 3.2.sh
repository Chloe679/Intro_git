...
lscpu | grep -E '^Core|^Socket|^CPU\(' # https://unix.stackexchange.com/questions/218074/how-to-know-number-of-cores-of-a-system-in-linux
echo "Total Memory: $(free -h --si | awk '/m:/ {print $2}')" # https://stackoverflow.com/questions/65871372/linux-find-os-disk-size-using-df-and-awk
echo "Total Disk Space: $(df -h --total | awk '/total/ {print $2}')"
echo
echo "# CURRENT STATE"
echo
mem_used=$(free | awk '/m:/ {printf "%.0f", $3/$2 * 100}')
mem_used_h=$(free -h --si | awk '/m:/ {print $3}')
